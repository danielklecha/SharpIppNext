...
public class PromoteJobRequestProfileTests
{
    ...
    [TestMethod]
    public void Map_PromoteJobRequest_ShouldSetCorrectOperation()
    {
        ...
        var result = mapper.Map<PromoteJobRequest, IppRequestMessage>(request);
        result.IppOperation.Should().Be(IppOperation.PromoteJob);
    }

    [TestMethod]
    public void Map_IppRequestMessageToPromoteJobRequest_ShouldNotThrow()
    {
        var mapper = CreateMapper();
        var msg = CreateBaseMessage(IppOperation.PromoteJob);
        Action act = () => mapper.Map<IIppRequestMessage, PromoteJobRequest>(msg);
        act.Should().NotThrow();
    }
}

...
internal class PrinterInputTrayProfile : IProfile
{
    public void CreateMaps(IMapperConstructor mapper)
    {
        mapper.CreateMap<IDictionary<string, IppAttribute[]>, PrinterInputTray>((src, map) =>
        {
            ...
            {
                Type = map.MapFromDicNullable<InputTrayType?>(src, "type"),
                ...
                MediaColor = map.MapFromDicNullable<MediaColor?>(src, "media-color"),
                MediaInfo = map.MapFromDicNullable<string?>(src, "media-info"),
                MediaType = map.MapFromDicNullable<MediaType?>(src, "media-type"),
                Unit = map.MapFromDicNullable<CapacityUnit?>(src, "unit"),
                FeedOrientation = map.MapFromDicNullable<FeedOrientation?>(src, "feed-orientation"),
            };
        });
        mapper.CreateMap<PrinterInputTray, IEnumerable<IppAttribute>>((src, _) =>
        {
            if (NoValue.IsNoValue(src))
                return new[] { new IppAttribute(Tag.NoValue, IppAttributeNames.PrinterInputTray, NoValue.Instance) };
            ...
            if (src.MediaColor != null) attrs.Add(new IppAttribute(Tag.Keyword, "media-color", src.MediaColor.Value.Value));
            if (src.MediaInfo != null) attrs.Add(new IppAttribute(Tag.TextWithoutLanguage, "media-info", src.MediaInfo));
            if (src.MediaType != null) attrs.Add(new IppAttribute(Tag.Keyword, "media-type", src.MediaType.Value.Value));
            if (src.Unit != null) attrs.Add(new IppAttribute(Tag.Keyword, "unit", src.Unit.Value.Value));
            if (src.FeedOrientation != null) attrs.Add(new IppAttribute(Tag.Keyword, "feed-orientation", src.FeedOrientation.Value.Value));
            return attrs;
        });
    }
}

...
internal class GetNextDocumentDataResponseProfile : IProfile
{
    public void CreateMaps(IMapperConstructor mapper)
    {
        mapper.CreateMap<IppResponseMessage, GetNextDocumentDataResponse>((src, map) =>
        {
            ...
            if (dst.OperationAttributes == null &&
                (operationAttributes.ContainsKey(JobAttribute.Compression) ||
                 operationAttributes.ContainsKey(JobAttribute.DocumentDataGetInterval) ||
                 operationAttributes.ContainsKey(JobAttribute.LastDocument) ||
                 operationAttributes.ContainsKey(DocumentAttribute.DocumentNumber)))
            {
                dst.OperationAttributes = new GetNextDocumentDataResponseOperationAttributes
                {
                    AttributesCharset = map.MapFromDicNullable<string?>(operationAttributes, JobAttribute.AttributesCharset) ?? "utf-8",
                    AttributesNaturalLanguage = map.MapFromDicNullable<string?>(operationAttributes, JobAttribute.AttributesNaturalLanguage) ?? "en"
                };
            }
        ...
        mapper.CreateMap<GetNextDocumentDataResponse, IppResponseMessage>((src, map) =>
        {
            ...
            if (src.OperationAttributes?.DocumentDataGetInterval.HasValue == true)
            ...
            if (src.OperationAttributes?.LastDocument.HasValue == true)
            ...
            if (src.OperationAttributes?.DocumentNumber.HasValue == true)

...
internal class GetNextDocumentDataResponseProfile : IProfile
{
    public void CreateMaps(IMapperConstructor mapper)
    {
        mapper.CreateMap<IppResponseMessage, GetNextDocumentDataResponse>((src, map) =>
        {
            ...
            dst.OperationAttributes.DocumentNumber = map.MapFromDicNullable<int?>(operationAttributes, IppAttributeNames.DocumentNumber);
        ...
        mapper.CreateMap<GetNextDocumentDataResponse, IppResponseMessage>((src, map) =>
        {
            ...
            if(src.OperationAttributes != null)
            {
                if (src.OperationAttributes.Compression != null)
                    dst.OperationAttributes[0].Add(new IppAttribute(Tag.Keyword, IppAttributeNames.Compression, map.Map<string>(src.OperationAttributes.Compression.Value)));
                if (src.OperationAttributes.DocumentDataGetInterval.HasValue)
                    dst.OperationAttributes[0].Add(new IppAttribute(Tag.Integer, IppAttributeNames.DocumentDataGetInterval, src.OperationAttributes.DocumentDataGetInterval.Value));
                if (src.OperationAttributes.LastDocument.HasValue)
                    dst.OperationAttributes[0].Add(new IppAttribute(Tag.Boolean, IppAttributeNames.LastDocument, src.OperationAttributes.LastDocument.Value));
                if (src.OperationAttributes.DocumentNumber.HasValue)
                    dst.OperationAttributes[0].Add(new IppAttribute(Tag.Integer, IppAttributeNames.DocumentNumber, src.OperationAttributes.DocumentNumber.Value));
            }
            return dst;
        });
    }

...
public class MapperApplierExtensionsTests
{
    ...
    [TestMethod]
    public void MapFromDicSet_Should_Return_Mapped_Values()
    {
        // Arrange
        var mapperMock = new Mock<IMapperApplier>();
        var src = new Dictionary<string, IppAttribute[]>
        ...
        mapperMock.Setup(m => m.Map<int[]?>(It.Is<object[]>(v => v.Length == 1 && (int)v[0] == 3)))
            .Returns(new[] { 3 });
    ...
    [TestMethod]
    public void MapFromDicSet_Should_Return_NoValue_For_Missing_Key()
    {
        var mapperMock = new Mock<IMapperApplier>();
        mapperMock.Setup(m => m.Map<int[]?>(NoValue.Instance)).Returns((int[]?)null);
        var src = new Dictionary<string, IppAttribute[]>();
